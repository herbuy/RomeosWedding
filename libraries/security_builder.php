...
class ServerVariables{
    ...
    public static function http_referer_domain()
    {
        $referer = self::http_referer();
        if(substr($referer,0,7) == "http://"){
            $referer = substr($referer,7);
        }
        else if(substr($referer,0,8) == "https://"){
            $referer = substr($referer,8);
        }
        else{

        }
        //at this point, we have removed any request scheme for the refere if it is http or https
        // now let us remove anything URI - the part after the first slash if any
        $first_index_of_a_slash = intval(strpos($referer,"/"));
        if($first_index_of_a_slash > 0){
            $referer = substr($referer,0, $first_index_of_a_slash);
        }
